...
- **限时冲刺**：60 秒听音打词连击挑战，音调随连击上升，服务端保留个人最高分；开局可召唤"幽灵"——个人最佳按均匀配速换算的实时对手，结算看胜负
- **每日挑战**：按日期+词库做种子，全站同题 10 道（音→形/音→义/义→形混合），每人每天一次计分机会；服务端重放题组权威判分；完成后生成 Wordle 式 emoji 网格分享图
- **错词 Boss 战**：错得最多的词打包成一只 Boss——听音打词，打对扣 Boss 血并把该词从错词本除名，打错扣心（共 3 颗）；战果一次性提交校验，喂经验但不碰记忆状态
- **英中配对消消乐**：词与释义翻牌两两配对、配上一对消一对；配错双方记失误并失去"首配即中"的满额经验，通关按用时/步数/失误结算星级
- **听音排句**：听句子音频把打乱的词块点回正确顺序，长句自动切成 ≤8 块；确定性发牌（同句全站同布局）服务端重拼判分，拼错亮出原句语序
...
| GET /api/daily?list= | 每日挑战题组（日期种子确定性出题，含当日已完成成绩） |
| POST /api/daily/result | 每日挑战交卷（服务端判分，每人每天仅首答计分） |
| GET /api/boss/session | 错词 Boss 战集结：错得最多的 n 个词（可 ?list= 聚焦） |
| POST /api/boss/result | Boss 战结算：答对清出错词本、写入 boss 练习桶 |
| GET /api/match/session | 配对消消乐发牌：n 个词的英中对（同文去重） |
| POST /api/match/result | 消消乐结算：首配即中数写入 match 练习桶 |
| GET /api/arrange/session | 听音排句出题：n 道打乱词块的句子（可 ?lesson= 聚焦） |
| POST /api/arrange/answer | 排句判分：按提交下标重拼比对原句，逐句记账 |
| GET /api/profile | 词力档案：经验/等级/称号 + 单词树状态 + 活跃统计（实时推导） |
| GET /api/wrong | 错词本 |
